...
export default function Media() {
  ...
  const addMedia = () => {
    ...
    setMediaList([...mediaList, { id: Date.now(), ...newItem } as MediaItem]);
    setNewItem({ type: "Book" });
  };

  const updateMedia = (id: number, key: keyof MediaItem, value: any) => {
    setMediaList(
      mediaList.map(item => (item.id === id ? { ...item, [key]: value } : item))
    );
  };

  const filteredList =
    filter === "All" ? mediaList : mediaList.filter(item => item.type === filter);

  return (
    <div className="p-6 font-serif text-black bg-white min-h-screen">
      <h2 className="text-3xl font-bold mb-2">Media Library</h2>
      <p className="mb-6 text-sm">
        Catalog your films, shows, books, music, and musicals. Track progress,
        rate, review, and note recommendations.
      </p>
        ...
        {filteredList.map(item => (
          ...
          >
            ...
            <input
              ...
              onChange={e =>
                updateMedia(item.id, "recommendedBy", e.target.value)
              }
